...
/**
 * <p>
 * Class providing graph building from given list of clients, depots and vehicle
 * speed. It also provides graph dividing depending on partitioning number
 * </p>
 * 
 * @author Filip
 */
public class Graph
{
	...
	/**
	 * <p>
	 * Returns number of combinations in graph partitioning
	 * </p>
	 */
	public static BigInteger calcNumOfPartitionings(int numOfClients,
			int numOfVehicles)
	{
		...
		numberOfPartitionings = BigInteger.valueOf(numOfVehicles)
				.pow(numOfClients - numOfVehicles).multiply(novFactorial);
	...
	// constructs graph, where nodes are in order: { d c1 ... ck } , d=depot,
	// ci=client
	...
	/**
	 * <p>
	 * Initialzes graph before partitioning.
	 * </p>
	 * 
	 * @param from
	 *            lower bound of the set of partitioning numbers
	 * @param to
	 *            higher bound of the set of partitioning numbers; if null its
	 *            set to max (numOfPatitionings)
	 */
	public void initialize(BigInteger from, BigInteger to, int numOfVehicles)
	{
		...
		this.numOfPartitionings = calcNumOfPartitionings(numOfClients,
				numOfVehicles);

		if (to != null)
			this.to = to;
		else
			this.to = this.numOfPartitionings;
	...
	/**
	 * <p>
	 * Divides calling graph into numOfVehicles parts. Every part contains all
	 * depots and some part of clients.
	 * </p>
	 * 
	 * @return Array of result graphs or null if partitioningNumber is wrong
	 * @throws IllegalStateException
	 *             when called uninitialized
	 */
	...
	private int[] partitioning()
	{
		...
		do
		{
			...
			for (int i = 0; i < numOfClients; i++)
			{
				...
				if (nextValue > currentMax) currentMax = nextValue;
